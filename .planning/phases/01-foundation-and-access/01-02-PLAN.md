...
must_haves:
  truths:
    ...
    - "Notification privacy utilities exist and can validate/sanitize notification content (COMP-03 scaffolding)"
  artifacts:
    - path: "middleware.ts"
    ...
    - path: "constants/notification-privacy.ts"
      provides: "Notification sanitization utilities for COMP-03"
      exports: ["sanitizeNotificationContent", "validateNotificationPayload", "buildSafeNotificationSubject"]
...
<task type="auto">
  ...
  <action>
    ...
    Create `constants/notification-privacy.ts` (COMP-03 scaffolding):
    - This file establishes the notification privacy pattern for the entire platform. Phase 1 does not send notifications, but this scaffolding ensures the pattern is available when Phase 2 adds email notifications.
    - Export NOTIFICATION_SAFE_FIELDS: string[] — whitelist of fields that may appear in notification content (e.g. 'organisationName', 'actionRequired', 'dateRange'). Health details, employee names, conditions, and diagnosis info are NEVER included.
    - Export NOTIFICATION_FORBIDDEN_PATTERNS: RegExp[] — patterns that must never appear in notification subject/body (e.g. /diagnosis/i, /condition/i, /illness/i, /symptom/i, /medication/i, /treatment/i, /disability/i)
    - Export `sanitizeNotificationContent(content: string): string` — strips any forbidden patterns from notification text, replacing with generic placeholders (e.g. "[details available in platform]")
    - Export `buildSafeNotificationSubject(type: string, metadata: Record<string, string>): string` — constructs a notification subject line using only safe fields. Example: "Action required: Review pending item" (never "John Smith is sick with flu")
    - Export `validateNotificationPayload(payload: { subject: string; body: string }): { valid: boolean; violations: string[] }` — checks payload against forbidden patterns, returns violations list
    - Add JSDoc comments explaining COMP-03: "Notifications must never reveal health details, employee names, or conditions in subject/body"
...
- COMP-03: Notification privacy scaffolding — sanitization, validation, and safe subject builder utilities established
